...
use crate::{
    OrdBag, OrdBiMap, OrdInsertionOrderMap, OrdInsertionOrderSet, OrdMap, OrdMultiMap, OrdSet,
    OrdSymMap, OrdTrie, Vector,
};
...
/// A strategy for an [`OrdBag`][OrdBag] of a given size.
///
/// The `size` range controls the total element count (including duplicates).
///
/// [OrdBag]: ../type.OrdBag.html
pub fn ord_bag<A: Strategy + 'static>(
    element: A,
    size: Range<usize>,
) -> BoxedStrategy<OrdBag<<A::Tree as ValueTree>::Value>>
where
    <A::Tree as ValueTree>::Value: Ord + Clone,
{
    vec(element, size.clone())
        .prop_map(OrdBag::from_iter)
        .prop_filter("OrdBag minimum size".to_owned(), move |b| {
            b.total_count() >= size.start
        })
        .boxed()
}

/// A strategy for an [`OrdMultiMap`][OrdMultiMap] of a given size.
///
/// The `size` range controls the total entry count (including duplicate keys).
///
/// [OrdMultiMap]: ../type.OrdMultiMap.html
pub fn ord_multi_map<K: Strategy + 'static, V: Strategy + 'static>(
    key: K,
    value: V,
    size: Range<usize>,
) -> BoxedStrategy<OrdMultiMap<<K::Tree as ValueTree>::Value, <V::Tree as ValueTree>::Value>>
where
    <K::Tree as ValueTree>::Value: Ord + Clone,
    <V::Tree as ValueTree>::Value: Ord + Clone,
{
    vec((key, value), size.clone())
        .prop_map(OrdMultiMap::from_iter)
        .prop_filter("OrdMultiMap minimum size".to_owned(), move |m| {
            m.len() >= size.start
        })
        .boxed()
}

/// A strategy for an [`OrdBiMap`][OrdBiMap] of a given size.
///
/// Both keys and values must be unique (OrdBiMap is a bijection). The `size`
/// range applies to distinct entries; duplicates are silently deduplicated.
///
/// [OrdBiMap]: ../type.OrdBiMap.html
pub fn ord_bimap<K: Strategy + 'static, V: Strategy + 'static>(
    key: K,
    value: V,
    size: Range<usize>,
) -> BoxedStrategy<OrdBiMap<<K::Tree as ValueTree>::Value, <V::Tree as ValueTree>::Value>>
where
    <K::Tree as ValueTree>::Value: Ord + Clone,
    <V::Tree as ValueTree>::Value: Ord + Clone,
{
    vec((key, value), size.clone())
        .prop_map(OrdBiMap::from_iter)
        .prop_filter("OrdBiMap minimum size".to_owned(), move |m| {
            m.len() >= size.start
        })
        .boxed()
}

/// A strategy for an [`OrdSymMap`][OrdSymMap] of a given size.
///
/// Pairs `(a, b)` are generated by drawing independently from `first` and
/// `second`. Both must be strategies over the same element type.
///
/// [OrdSymMap]: ../type.OrdSymMap.html
pub fn ord_symmap<A: Strategy + 'static>(
    first: A,
    second: A,
    size: Range<usize>,
) -> BoxedStrategy<OrdSymMap<<A::Tree as ValueTree>::Value>>
where
    <A::Tree as ValueTree>::Value: Ord + Clone,
{
    vec((first, second), size.clone())
        .prop_map(OrdSymMap::from_iter)
        .prop_filter("OrdSymMap minimum size".to_owned(), move |m| {
            m.len() >= size.start
        })
        .boxed()
}

/// A strategy for an [`OrdTrie`][OrdTrie] of a given size.
///
/// Each entry is a `(Vec<K>, V)` pair where `Vec<K>` is the path. The
/// `path_len` range controls the length of each generated path; `size`
/// controls the number of entries.
///
/// [OrdTrie]: ../type.OrdTrie.html
pub fn ord_trie<K: Strategy + 'static, V: Strategy + 'static>(
    key: K,
    value: V,
    path_len: Range<usize>,
    size: Range<usize>,
) -> BoxedStrategy<OrdTrie<<K::Tree as ValueTree>::Value, <V::Tree as ValueTree>::Value>>
where
    <K::Tree as ValueTree>::Value: Ord + Clone,
    <V::Tree as ValueTree>::Value: Clone,
{
    vec((vec(key, path_len), value), size.clone())
        .prop_map(OrdTrie::from_iter)
        .prop_filter("OrdTrie minimum size".to_owned(), move |t| {
            t.len() >= size.start
        })
        .boxed()
}

/// A strategy for an [`OrdInsertionOrderMap`][OrdInsertionOrderMap] of a given size.
///
/// [OrdInsertionOrderMap]: ../type.OrdInsertionOrderMap.html
pub fn ord_insertion_order_map<K: Strategy + 'static, V: Strategy + 'static>(
    key: K,
    value: V,
    size: Range<usize>,
) -> BoxedStrategy<OrdInsertionOrderMap<<K::Tree as ValueTree>::Value, <V::Tree as ValueTree>::Value>>
where
    <K::Tree as ValueTree>::Value: Ord + Clone,
    <V::Tree as ValueTree>::Value: Clone,
{
    vec((key, value), size.clone())
        .prop_map(OrdInsertionOrderMap::from_iter)
        .prop_filter("OrdInsertionOrderMap minimum size".to_owned(), move |m| {
            m.len() >= size.start
        })
        .boxed()
}

/// A strategy for an [`OrdInsertionOrderSet`][OrdInsertionOrderSet] of a given size.
///
/// [OrdInsertionOrderSet]: ../type.OrdInsertionOrderSet.html
pub fn ord_insertion_order_set<A: Strategy + 'static>(
    element: A,
    size: Range<usize>,
) -> BoxedStrategy<OrdInsertionOrderSet<<A::Tree as ValueTree>::Value>>
where
    <A::Tree as ValueTree>::Value: Ord + Clone,
{
    vec(element, size.clone())
        .prop_map(OrdInsertionOrderSet::from_iter)
        .prop_filter("OrdInsertionOrderSet minimum size".to_owned(), move |s| {
            s.len() >= size.start
        })
        .boxed()
}
...
#[cfg(any(feature = "std", feature = "foldhash"))]
/// A strategy for a [`UniqueVector`][UniqueVector] of a given size.
///
/// Duplicate elements are silently deduplicated. The `size` range applies
/// to the number of distinct elements in the resulting collection.
///
/// [UniqueVector]: ../type.UniqueVector.html
pub fn unique_vector<A: Strategy + 'static>(
    element: A,
    size: Range<usize>,
) -> BoxedStrategy<UniqueVector<<A::Tree as ValueTree>::Value>>
where
    <A::Tree as ValueTree>::Value: Hash + Eq + Clone,
{
    vec(element, size.clone())
        .prop_map(UniqueVector::from_iter)
        .prop_filter("UniqueVector minimum size".to_owned(), move |v| {
            v.len() >= size.start
        })
        .boxed()
}
